...
public class CmdLineRunner {
	...
	public static Object cmdLineRun(
		...
		if (simuCtx != null) {
		  	if (runStudyCase(simuCtx, runType))
		  		outputResult(simuCtx, runType, outFilename);
		}
		return simuCtx;
	}
	...
	private static SimuContext inputData(String filename) {
		try {
			...
			SimuContext simuCtx = adapter.load(filename, SpringAppContext.getIpssMsgHub());
		  	return simuCtx;
		} catch (Exception e) {
			IpssLogger.logErr(e);
			return null;
	...
	private static void outputResult(SimuContext simuCtx, String runType, String outFilename) {
