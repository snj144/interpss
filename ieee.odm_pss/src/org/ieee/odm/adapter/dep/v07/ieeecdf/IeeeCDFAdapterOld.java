...
package org.ieee.odm.adapter.dep.v07.ieeecdf;
	...
	private  void processBusData(final String str, JaxbODMModelParser parser) {
		...
		if (max != 0.0 || min != 0.0) {
			...
			if (type == 1) {
				...
				busData.getGenData().getEquivGen().setQLimit(BaseDataSetter.createReactivePowerLimit(max, min, ReactivePowerUnitType.MVAR));
