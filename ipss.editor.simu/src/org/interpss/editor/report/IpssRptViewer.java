...
*/			
	...
	public void loadReport(JasperPrint jrPrint) {
		...
		try {
			...
			IpssLogger.logErr(e);
			SpringAppContext.getEditorDialogUtil().showMsgDialog("InterPSS Report Error", e.toString());
		}	
	}
	...
	public void exportActionPerformed() {
		...
		if (retValue == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				{
					if (lowerCaseFileName.endsWith(".pdf")  && pdfSaveContrib != null) {
	...
	private JButton createButton(String iconFile, String helpTip) {
		...
		btn.setIcon(new ImageIcon(getClass().getClassLoader().getResource(iconFile)));
