...
public class VStabObjectFactory {
	...
	public static CPFAlgorithm createCPFAlgorithmImpl(AclfNetwork net,LoadIncrease loadInc,GenDispatch genDisp) {
		LambdaParam lambda = new LambdaParam(net.getNoBus(),0.01);// index is changed, now normally it is 0->n-1, so the lambda should be the Nth paramether; 
		CPFAlgorithm cpf=new CPFAlgorithmImpl(net, lambda, loadInc,genDisp);
		return cpf;
	}
