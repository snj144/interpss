...
public class NBBusEditDialog extends javax.swing.JDialog  implements IFormDataDialog {
	...
	*/
	public NBBusEditDialog(java.awt.Frame parent, IPSSMsgHub aMsgHub) {
        super(parent, java.util.ResourceBundle.getBundle("org/interpss/editor/resources/Locale").getString("NBBusEditDialog.Form.Title"), true);
        initComponents();
	...
	*/
	public boolean setForm2Editor() {
      this.busNumberLabel.setText(Translator.getString("Common.Bus")+" [" + _form.getId() + "]");
		...
		this.baseVoltComboBox.setSelectedItem(Number2String.toStr(
				_form.getBaseVoltage(), "#0"+((DecimalFormat)NumberFormat.getInstance()).getDecimalFormatSymbols().getDecimalSeparator() +"00"));
	...
	*/
    public boolean saveEditor2Form(Vector errMsg) throws Exception {
    	...
    	if (!NetUtilFunc.isValidBusid(busNameField.getText())) {
			errMsg.add(Translator.getString("NBBusEditDialog.Error.BusNameEmpty"));
			ok = false;
    	}
        else if (_netContainer.hasBusName(_form.getId(), busNameField.getText())) {
			errMsg.add(Translator.getString("NBBusEditDialog.Error.BusNameExists"));
			ok = false;
    	}
		...
		if (!SwingInputVerifyUtil.largeThan(this.baseVoltComboBox, 0.0d)) {
			errMsg.add(Translator.getString("NBBusEditDialog.Error.BusBaseVoltage"));
			ok = false;
		}
    ...
    //After saving form in NetBeans add 
    //Translator.pushBundle("org/interpss/editor/resources/Locale");
    ...
    private void initComponents() {
        ...
        busNumberLabel.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        busNumberLabel.setText(Translator.getString("Common.Bus"+"#: 0001"));
        idPanel.add(busNumberLabel);

        busNameLabel.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        busNameLabel.setText("   "+Translator.getString("Common.BusName"));
        idPanel.add(busNameLabel);

        busNameField.setColumns(10);
        busNameField.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        busNameField.setName("busNameTextField"); // NOI18N
        idPanel.add(busNameField);

        baseVoltLabel.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        baseVoltLabel.setText("   "+Translator.getString("Common.BaseVolt"));
        idPanel.add(baseVoltLabel);

        baseVoltComboBox.setEditable(true);
        baseVoltComboBox.setFont(new java.awt.Font("Dialog", 0, 10));
        baseVoltComboBox.setName("baseVoltComboBox"); // NOI18N
        baseVoltComboBox.setPreferredSize(new java.awt.Dimension(80, 25));
        idPanel.add(baseVoltComboBox);

        baseUnitComboBox.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        baseUnitComboBox.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Volt", "KV" }));
        baseUnitComboBox.setName("baseUnitComboBox"); // NOI18N
        idPanel.add(baseUnitComboBox);

        areaLabel.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        ...
        zoneLabel.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
        ...
        inServiceCheckBox.setFont(new java.awt.Font("Dialog", 0, 10)); // NOI18N
    ...
    private void saveActionHandler(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionHandler
		...
		try {
        	if (!saveEditor2Form(errMsg)) {
      		...
      		IpssLogger.logErr(e);
      		CoreCommonSpringCtx.getEditorDialogUtil().showMsgDialog(this, Translator.getString("NBBusEditDialog.Error.InputError"), e.toString());
			return;
        }	
