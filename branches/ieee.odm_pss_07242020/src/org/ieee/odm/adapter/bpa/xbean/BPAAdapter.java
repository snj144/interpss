...
package org.ieee.odm.adapter.bpa.xbean;
...
import org.ieee.odm.adapter.bpa.xbean.impl.BPABranchRecord;
import org.ieee.odm.adapter.bpa.xbean.impl.BPABusRecord;
import org.ieee.odm.adapter.bpa.xbean.impl.BPADynamicRecord;
...
public class BPAAdapter  extends AbstractODMAdapter {
	...
	private void processNetData(final String str,final NameValuePairListXmlType nvList,
			...
			if (strAry[0]!= null ){
				XBeanParserHelper.addNVPair(nvList, strAry[0], strAry[1]);
				getLogger().fine(strAry[0] +": " + strAry[1]);
			}
			
			if (strAry[2]!= null ){
				XBeanParserHelper.addNVPair(nvList, strAry[2], strAry[4]);
	...
	private void processAreaData(final String str,final XBeanODMModelParser parser ,
		...
		if(str.trim().startsWith("A")||str.trim().startsWith("AC")){	
            ...
            if(!strAry[5].equals("")){            	
            	XBeanDataSetter.setPowerData(area.addNewTotalExchangePower(),
           ...
           while((!strAry[6].substring(i, i+2).equals(""))&& i+2<=Str6length){            		
			...
			if(!strAry[2].trim().equals("")){
				...
				NetAreaXmlType area=XBeanParserHelper.
