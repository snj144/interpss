...
public class BPABusRecord {
	...
	public static void processBusData(final String str,
			final BusRecordXmlType busRec, BPAAdapter adapter) {
		...
		final int pvBus = 3;
		// busType
		if (strAry[0].equals("B") || strAry[0].equals("BC")
				|| strAry[0].equals("BT") || strAry[0].equals("BV")) {
			busType = pqBus;
		} else if (strAry[0].equals("BE") || strAry[0].equals("BQ")
				|| strAry[0].equals("BG") || strAry[0].equals("BF")) {
			busType = pvBus;
		...
		ODMData2XmlHelper.setVoltageData(busRec.addNewBaseVoltage(), baseKv,
		...
		if (loadMw != 0.0 || loadMvar != 0.0 || pGen != 0.0
				|| qGenOrQGenMax != 0.0 || vMinOrAngDeg != 0.0
				|| pGenMax != 0.0 || shuntMw != 0.0 || shuntVar != 0.0) {
			LoadflowBusDataXmlType busData = busRec.addNewLoadflowBusData();
			// set G B
			if (shuntMw != 0.0 || shuntVar != 0.0) {
				ODMData2XmlHelper.setYData(busData.addNewShuntY(), shuntMw,
						shuntVar, YXmlType.Unit.MHO);
			}
			// set load
			...
			if (busType == swingBus) {
				...
				if (vpu != 0.0) {
					...
					ODMData2XmlHelper.setVoltageData(busData.addNewVoltage(),
							vpu, VoltageXmlType.Unit.PU);
				}
				// set bus angle
				ODMData2XmlHelper.setAngleData(busData.addNewAngle(),
						vMinOrAngDeg, AngleXmlType.Unit.DEG);
				...
				if (qGenOrQGenMax != 0.0 || qGenMin != 0.0) {
					ODMData2XmlHelper.setGenQLimitData(busData.getGenData(),
							qGenOrQGenMax, qGenMin,
							GenDataXmlType.QGenLimit.QLimitUnit.MVAR);
				}
				// set P limit
				if (pGenMax != 0.0) {
					if (busData.getGenData().getGen().getPGenLimit() == null) {
						busData.getGenData().getGen().addNewPGenLimit();
					}
					ODMData2XmlHelper
							.setLimitData(busData.getGenData().getGen()
									.getPGenLimit().addNewPLimit(), pGenMax, 0);
				}
			} else if (busType == pqBus) {
				if (pGen != 0.0 || qGenOrQGenMax != 0.0) {
					ODMData2XmlHelper.setGenData(busData,
							LoadflowBusDataXmlType.GenData.Code.PQ, pGen,
							qGenOrQGenMax, PowerXmlType.Unit.MVA);
				}
				// set V limit
				if (vpu != 0 || vMinOrAngDeg != 0) {
					...
					ODMData2XmlHelper.setLimitData(busData.getGenData()
							.getGen().getVGenLimit().addNewVLimit(), vpu,
							vMinOrAngDeg);
				}
			...
			if (strAry[0].equals("BG") || strAry[0].equals("BX")) {
				if (!controlledBus.equals("")) {
					...
					ODMData2XmlHelper.setVoltageData(busData.getGenData()
							.getGen().getDesiredRemoteVoltage()
							.addNewDesiredVoltage(), vpu,
	...
	public static void processDCLineBusData(final String str,
			final DCLineBusRecordXmlType dcBus, BPAAdapter adapter) {
		...
		// set smooth reactor
		ODMData2XmlHelper.setZValue(converter.addNewSmoothingReactor(), 0.0,
				smoothReactance, ZXmlType.Unit.OHM);
		// set min firing angle as a converter
		ODMData2XmlHelper.setAngleData(converter
				.addNewRectifierMinFiringAngle(), converterMinFiringAngle,
				AngleXmlType.Unit.DEG);
		// set max firing angle as a inverter
		ODMData2XmlHelper.setAngleData(
				converter.addNewInverterMaxFiringAngle(),
	...
	private static String[] getBusDataFields(final String str,
		...
		try {
			...
			strAry[5] = StringUtil.getStringReturnEmptyString(str, 19, 20)
					.trim();

			// Columns 20-24 Load MW [F] *
			// Columns 25-29 Load MVAR [F] *
			strAry[6] = StringUtil.getStringReturnEmptyString(str, 21, 25)
					.trim();

			strAry[7] = StringUtil.getStringReturnEmptyString(str, 26, 30)
					.trim();
			// Columns 30-33 shunt MW [F] *
			// Columns 34-39 shunt MVAR [F] *
			strAry[8] = StringUtil.getStringReturnEmptyString(str, 31, 34)
					.trim();
			strAry[9] = StringUtil.getStringReturnEmptyString(str, 35, 38)
					.trim();

			// Columns 38-41 pmax
			// Columns 42-46 pmax
			strAry[10] = StringUtil.getStringReturnEmptyString(str, 39, 42)
					.trim();

			strAry[11] = StringUtil.getStringReturnEmptyString(str, 43, 47)
					.trim();

			// Qmax Qmin
			strAry[12] = StringUtil.getStringReturnEmptyString(str, 48, 52)
					.trim();
			strAry[13] = StringUtil.getStringReturnEmptyString(str, 53, 57)
					.trim();
			// scheduled V or Vmax, Vmin
			strAry[14] = StringUtil.getStringReturnEmptyString(str, 58, 61)
					.trim();
			strAry[15] = StringUtil.getStringReturnEmptyString(str, 62, 65)
					.trim();

			// remoted busName, rated voltage
			strAry[16] = StringUtil.getStringReturnEmptyString(str, 66, 73)
					.trim();
	...
	private static String[] getDCLineBusDataFields(final String str,
		...
		try {
			...
			// bridge per brckt
			strAry[6] = StringUtil.getStringReturnEmptyString(str, 24, 25)
					.trim();
			// smooth reactor
			strAry[7] = StringUtil.getStringReturnEmptyString(str, 26, 30)
					.trim();

			strAry[8] = StringUtil.getStringReturnEmptyString(str, 31, 35)
					.trim();

			strAry[9] = StringUtil.getStringReturnEmptyString(str, 36, 40)
					.trim();
			// Columns 38-41 pmax
			// Columns 42-46 pmax
			strAry[10] = StringUtil.getStringReturnEmptyString(str, 41, 45)
					.trim();
			strAry[11] = StringUtil.getStringReturnEmptyString(str, 46, 50)
					.trim();
			//Qmax Qmin
			strAry[12] = StringUtil.getStringReturnEmptyString(str, 51, 58)
					.trim();
			strAry[13] = StringUtil.getStringReturnEmptyString(str, 59, 62)
					.trim();

		} catch (Exception e) {
			adapter.logErr("This DCLine bus data is not filled completely:   "
					+ str);
		}
		return strAry;
	}
