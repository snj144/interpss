...
public class BPABranchRecord {
	public static void processBranchData(final String str,
			final BranchRecordXmlType branchRec, IEEEODMPSSModelParser parser,
		...
		if (str.startsWith("L")) {
			...
			adapter.getLogger()
					.fine(
							"Branch data loaded, from-Bus, to-Bus: " + fid
									+ ", " + tid);
			if (!fid.equals("")) {
			...
			if (!strAry[5].equals("")) {
				...
				try {
					if (measureLocation == 1) {
						PSSNetworkXmlType.TieLineList.Tieline tieLine = parser
								.addNewBaseCaseTieline();
						// set tieline data
						tieLine.addNewMeteredBus().setName(fid);
						tieLine.addNewNonMeteredBus().setName(tid);

						BusRecordXmlType busRecFrom = ODMData2XmlHelper
								.getBusRecord(fid, baseCaseNet);
						busRecFrom.getZone();
						PSSNetworkXmlType.AreaList.Area areaFrom = ODMData2XmlHelper
								.getAreaRecordByZone(busRecFrom.getZone(),
										baseCaseNet);
						tieLine.setMeteredArea(areaFrom.getAreaName());

						BusRecordXmlType busRecTo = ODMData2XmlHelper
								.getBusRecord(tid, baseCaseNet);
						busRecTo.getZone();
						PSSNetworkXmlType.AreaList.Area areaTo = ODMData2XmlHelper
								.getAreaRecordByZone(busRecTo.getZone(),
										baseCaseNet);
						...
						ODMData2XmlHelper.getBusRecord(fid, baseCaseNet)
								.getZone();
			...
			branchRec.getLoadflowBranchData().setCode(
					LoadflowBranchDataXmlType.Code.LINE);
			...
			if (!strAry[10].equals("")) {
				...
				ODMData2XmlHelper
						.setBranchRatingLimitData(
								branchRec.getLoadflowBranchData(),
								currentRating,
								LoadflowBranchDataXmlType.RatingLimit.CurrentRatingUnit.AMP);
			}
			...
			if (rpu != 0.0 || xpu != 0.0 || halfGpu != 0.0 || halfBpu != 0.0) {
				ODMData2XmlHelper.setLineData(
						branchRec.getLoadflowBranchData(), rpu, xpu,
						ZXmlType.Unit.PU, 2 * halfGpu, 2 * halfBpu,
						YXmlType.Unit.PU);
			}
			...
			if (!strAry[16].equals("")) {
				...
				branchRec.getLoadflowBranchData().getLineData().addNewLength()
						.setLength(length);
				branchRec.getLoadflowBranchData().getLineData().getLength()
						.setUnit(LengthXmlType.Unit.MILE);
			}
			// if there is a description, set
			String desc = "";
			if (!strAry[17].equals("")) {
				desc = strAry[17];
				NameValuePairXmlType nvPair = branchRec.getLoadflowBranchData()
						.getLineData().addNewNvPairList().addNewNvPair();
			...
			ZXmlType z = branchRec.getLoadflowBranchData().getLineData()
					.addNewZ();
			ODMData2XmlHelper.setZValue(z, rpu, xpu, ZXmlType.Unit.PU);
			YXmlType y1 = branchRec.getLoadflowBranchData().getLineData()
					.addNewFromShuntY();
			YXmlType y2 = branchRec.getLoadflowBranchData().getLineData()
					.addNewToShuntY();
	...
	public static void processXfrData(final String str,
			final BranchRecordXmlType branchRec, IEEEODMPSSModelParser parser,
			final PSSNetworkXmlType baseCaseNet, BPAAdapter adapter) {
		...
		if (dataType == transformer) {
			branchRec.getLoadflowBranchData().setCode(
					LoadflowBranchDataXmlType.Code.TRANSFORMER);
			branchRec.getLoadflowBranchData().addNewXformerData();
		} else {
			branchRec.getLoadflowBranchData().setCode(
					LoadflowBranchDataXmlType.Code.PHASE_SHIFT_XFORMER);
		...
		final String fid = strAry[3];
		final String tid = strAry[6];
		adapter.getLogger().fine(
				"Branch data loaded, from-bus, to-bus: " + fid + ", " + tid);
		branchRec.addNewFromBus().setIdRef(fid);
		branchRec.addNewToBus().setIdRef(tid);
		...
		// set tieline data, measure location for power interchange, 1--from
		// side, 2- to side
		int measureLocation = 0;
		if (!strAry[5].equals("")) {
			measureLocation = new Integer(strAry[5]).intValue();
			try {
				if (measureLocation == 1) {
					...
					BusRecordXmlType busRecTo = ODMData2XmlHelper.getBusRecord(
							tid, baseCaseNet);
					PSSNetworkXmlType.AreaList.Area areaTo = ODMData2XmlHelper
							.getAreaRecordByZone(busRecTo.getZone(),
									baseCaseNet);
		...
		// set xfr rating data
		if (dataType == transformer) {
			ODMData2XmlHelper.setXfrRatingData(branchRec
					.getLoadflowBranchData().getXformerData(), fVol, tVol,
					VoltageXmlType.Unit.KV, MwRating, PowerXmlType.Unit.MVA);
		} else {
			ODMData2XmlHelper.setXfrRatingData(branchRec
					.getLoadflowBranchData().getPhaseShiftXfrData(), fVol,
					tVol, VoltageXmlType.Unit.KV, MwRating,
		...
		if (rpu != 0.0 || xpu != 0.0) {
			if (dataType == transformer) {
				ODMData2XmlHelper
						.setZValue(branchRec.getLoadflowBranchData()
								.getXformerData().addNewZ(), rpu, xpu,
								ZXmlType.Unit.PU);
			} else {
				ODMData2XmlHelper.setZValue(branchRec.getLoadflowBranchData()
						.getPhaseShiftXfrData().addNewZ(), rpu, xpu,
						ZXmlType.Unit.PU);
			}
		}
		...
		if (Gpu != 0.0 || Bpu != 0.0) {
			if (dataType == transformer) {
				ODMData2XmlHelper.setYData(branchRec.getLoadflowBranchData()
						.getXformerData().addNewFromShuntY(), Gpu, Bpu,
						YXmlType.Unit.PU);
			} else {
				ODMData2XmlHelper.setYData(branchRec.getLoadflowBranchData()
						.getPhaseShiftXfrData().addNewFromShuntY(), Gpu, Bpu,
						YXmlType.Unit.PU);
			}
		}
		...
		if (dataType == transformer) {
			...
			branchRec.getLoadflowBranchData().getXformerData().setToTurnRatio(
					tRatio);
		} else {
			ODMData2XmlHelper.setAngleData(branchRec.getLoadflowBranchData()
					.getPhaseShiftXfrData().addNewFromAngle(),
					fromTurnRatedVolOrAngDeg, AngleXmlType.Unit.DEG);
			ODMData2XmlHelper.setAngleData(branchRec.getLoadflowBranchData()
					.getPhaseShiftXfrData().addNewToAngle(), 0,
	...
	public static void processXfrAdjustData(final String str,
		...
		BranchRecordXmlType branchRec = ODMData2XmlHelper.getXfrBranchRecord(
				fromBus, toBus, baseCaseNet);
		...
		if (dataType == tapAdjustment) {
			...
			ODMData2XmlHelper.setLimitData(tapAdj.addNewTapLimit(), max, min);
			...
			if (adjustType == tapVoltageAdjustment) {// voltage control
				...
				if (maxVoltPQ != 0.0 || minVoltPQ != 0.0) {
					ODMData2XmlHelper.setLimitData(voltTapAdj
							.addNewDesiredRange(), maxVoltPQ, minVoltPQ);
				}
			...
			ODMData2XmlHelper.setLimitData(angAdj.addNewAngleDegLimit(), max,
					min);
			ODMData2XmlHelper.setLimitData(angAdj.addNewDesiredRange(),
					maxVoltPQ, minVoltPQ);
	...
	public static void processDCLineBranchData(final String str,
			...
			IEEEODMPSSModelParser parser, final PSSNetworkXmlType baseCaseNet,
			BPAAdapter adapter) {
		...
		if (!strAry[5].equals("")) {
			...
			try {
				if (measureLocation == 1) {
					...
					DCLineBusRecordXmlType busRecFrom = ODMData2XmlHelper
							.getDCLineBusRecord(rectifierBus, baseCaseNet);
					if (busRecFrom.getConverter().getZone() != null) {
						PSSNetworkXmlType.AreaList.Area areaFrom = ODMData2XmlHelper
								.getAreaRecordByZone(busRecFrom.getConverter()
										.getZone(), baseCaseNet);

						tieLine.setMeteredArea(areaFrom.getAreaName());
					}
					DCLineBusRecordXmlType busRecTo = ODMData2XmlHelper
							.getDCLineBusRecord(inverterBus, baseCaseNet);

					if (busRecTo.getConverter().getZone() != null) {

						PSSNetworkXmlType.AreaList.Area areaTo = ODMData2XmlHelper
								.getAreaRecordByZone(busRecTo.getConverter()
										.getZone(), baseCaseNet);
						tieLine.setNonMeteredArea(areaTo.getAreaName());
					}
					...
					ODMData2XmlHelper.getBusRecord(rectifierBus, baseCaseNet)
							.getZone();

					BusRecordXmlType busRecFrom = ODMData2XmlHelper
							.getBusRecord(inverterBus, baseCaseNet);
					busRecFrom.getZone();
					PSSNetworkXmlType.AreaList.Area areaFrom = ODMData2XmlHelper
							.getAreaRecordByZone(busRecFrom.getZone(),
									baseCaseNet);
		...
		if (r != 0.0 || x != 0.0) {
			ODMData2XmlHelper.setZValue(dcBranch.addNewLineZ(), r, x,
					ZXmlType.Unit.OHM);
		}
		...
		if (!strAry[13].equals("")) {
			...
			dcBranch
					.setControlMode(DCLineBranchRecordXmlType.ControlMode.POWER);
		...
		if (!strAry[15].equals("")) {
			...
			ODMData2XmlHelper.setAngleData(ODMData2XmlHelper
					.getConverterRecord(rectifierBus, baseCaseNet)
					.addNewRectifierOperAngle(), recOperFiringAngle,
					AngleXmlType.Unit.DEG);
		...
		if (!strAry[16].equals("")) {
			...
			ODMData2XmlHelper.setAngleData(ODMData2XmlHelper
					.getConverterRecord(inverterBus, baseCaseNet)
					.addNewInverterStopAgnle(), invStopFiringAngle,
					AngleXmlType.Unit.DEG);
			ODMData2XmlHelper.getConverterRecord(rectifierBus, baseCaseNet)
	...
	private static String[] getBranchDataFields(final String str,
		...
		if (strAry[0] == "L") {
			strAry[16] = StringUtil.getStringReturnEmptyString(str, 63, 66)
					.trim();
			strAry[17] = StringUtil.getStringReturnEmptyString(str, 67, 74)
					.trim();
		} else {
			strAry[16] = StringUtil.getStringReturnEmptyString(str, 63, 67)
					.trim();
			strAry[17] = StringUtil.getStringReturnEmptyString(str, 69, 74)
					.trim();
		}
	...
	private static String[] getXformerDataFields(final String str,
		...
		try {
			...
			strAry[3] = StringUtil.getStringReturnEmptyString(str, 7, 14)
					.trim();
			strAry[4] = StringUtil.getStringReturnEmptyString(str, 15, 18)
					.trim();
			strAry[5] = StringUtil.getStringReturnEmptyString(str, 19, 19)
					.trim();
			strAry[6] = StringUtil.getStringReturnEmptyString(str, 20, 27)
					.trim();

			strAry[7] = StringUtil.getStringReturnEmptyString(str, 28, 31)
					.trim();
			strAry[8] = StringUtil.getStringReturnEmptyString(str, 32, 32)
					.trim();
			strAry[9] = StringUtil.getStringReturnEmptyString(str, 33, 33)
					.trim();
			strAry[10] = StringUtil.getStringReturnEmptyString(str, 34, 37)
					.trim();
			strAry[11] = StringUtil.getStringReturnEmptyString(str, 38, 38)
					.trim();
			strAry[12] = StringUtil.getStringReturnEmptyString(str, 39, 44)
					.trim();
			strAry[13] = StringUtil.getStringReturnEmptyString(str, 45, 50)
					.trim();
			strAry[14] = StringUtil.getStringReturnEmptyString(str, 51, 56)
					.trim();
			strAry[15] = StringUtil.getStringReturnEmptyString(str, 57, 62)
					.trim();
			strAry[16] = StringUtil.getStringReturnEmptyString(str, 63, 67)
					.trim();

			strAry[17] = StringUtil.getStringReturnEmptyString(str, 68, 72)
					.trim();
	...
	private static String[] getXfrAdjustDataFields(final String str,
		...
		try {
			...
			strAry[8] = StringUtil.getStringReturnEmptyString(str, 34, 41)
					.trim();
			strAry[9] = StringUtil.getStringReturnEmptyString(str, 42, 45)
					.trim();

			// for R RV RQ RN
			// max tap
			strAry[10] = StringUtil.getStringReturnEmptyString(str, 46, 50)
					.trim();
			// min tap
			strAry[11] = StringUtil.getStringReturnEmptyString(str, 51, 55)
					.trim();
			// total tap
			strAry[12] = StringUtil.getStringReturnEmptyString(str, 56, 57)
					.trim();
	...
	private static String[] getDCLineBranchDataFields(final String str,
		...
		try {
			...
			strAry[8] = StringUtil.getStringReturnEmptyString(str, 34, 37)
					.trim();
			strAry[9] = StringUtil.getStringReturnEmptyString(str, 38, 43)
					.trim();
			strAry[10] = StringUtil.getStringReturnEmptyString(str, 44, 49)
					.trim();
			strAry[11] = StringUtil.getStringReturnEmptyString(str, 50, 55)
					.trim();
			strAry[12] = StringUtil.getStringReturnEmptyString(str, 56, 56)
					.trim();
			strAry[13] = StringUtil.getStringReturnEmptyString(str, 57, 61)
					.trim();
			strAry[14] = StringUtil.getStringReturnEmptyString(str, 62, 66)
					.trim();
			strAry[15] = StringUtil.getStringReturnEmptyString(str, 67, 70)
					.trim();
			strAry[16] = StringUtil.getStringReturnEmptyString(str, 71, 74)
					.trim();

			strAry[17] = StringUtil.getStringReturnEmptyString(str, 75, 78)
					.trim();
		} catch (Exception e) {
			adapter.logErr(e.toString());
		}
