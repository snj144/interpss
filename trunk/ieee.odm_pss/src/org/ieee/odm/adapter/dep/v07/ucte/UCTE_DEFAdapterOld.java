...
package org.ieee.odm.adapter.dep.v07.ucte;
...
public class UCTE_DEFAdapterOld extends AbstractODMAdapter {
	 ...
	 */
	public UCTE_DEFAdapterOld(Logger logger) {
		super(logger);
	}
    ...
    private void processNodeRecord(String str, String isoId, int busCnt, JaxbODMModelParser parser) throws Exception {
		...
		busData.setVoltage(BaseDataSetter.createVoltageValue(voltage, VoltageUnitType.KV));

		busData.setAngle(BaseDataSetter.createAngleValue(0.0, AngleUnitType.DEG));    	
		...
		case 2: // PV bus
			...
			if (((maxGenMVar != 0.0) || (minGenMVar != 0.0))
				...
				busData.getGenData().getEquivGen().setQLimit(BaseDataSetter.createReactivePowerLimit(  
    ...
    private void processXfr2WRegulationRecord(String str, JaxbODMModelParser parser) 
		...
		branchRec.setNvPairList(nvList);
      	...
      	if (dUPhase > 0.0) {
			...
			branchData.setToTurnRatio(BaseDataSetter.createTurnRatioPU(ratioFactor/x));
			...
			branchData.setToAngle(BaseDataSetter.createAngleValue(-ang*ModelContansts.Rad2Deg, AngleUnitType.DEG));
			branchData.setToTurnRatio(BaseDataSetter.createTurnRatioPU(ratioFactor/x));
