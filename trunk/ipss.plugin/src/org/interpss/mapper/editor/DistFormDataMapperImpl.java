...
public class DistFormDataMapperImpl {
	...
	private static DistNetwork createDistNet(GNetForm netForm) {
		...
		// multi-point SC data
		net
				.setScStd(netData.getScStd().equals(DistNetData.ScStd_Generic) ? ScStanderd.GENERIC
	...
	private static DistBus addBusForm2Net(GBusForm form, DistNetwork distNet) {
		...
		// Non-Contribute
		bus
				.setBusCode(busData.getBusCode().equals(
	...
	private static DistBranch addBranchForm2Net(GBranchForm branchForm,
		...
		branch
				.setBranchCode(branchData.getBranchCode().equals(
		...
		if (!branchData.getToXfrConnectData().getGrounding().getUnit()
				.toUpperCase().equals("PU")) {
