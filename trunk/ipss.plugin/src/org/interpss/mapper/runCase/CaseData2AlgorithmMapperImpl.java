...
public class CaseData2AlgorithmMapperImpl {
	 ...
	 * @param algo
	 */
	public static boolean acscCaseData2AlgoMapping(AcscCaseData caseData,
			SimpleFaultAlgorithm algo) {
		SimpleFaultNetwork faultNet = algo.getSimpleFaultNetwork();
		String faultIdStr = algo.getDesc();
		if (caseData.getFaultData().getType().equals(
				AcscFaultData.FaultType_BusFault)) {
			...
			if (caseData.getFaultData().getCategory().equals(
					AcscFaultData.FaultCaty_Fault_All)) {
				addAllFaultCategory(faultBus.getId(), faultIdStr, fault,
						faultNet);
			} else
		...
		algo.setScBusVoltage(caseData.getBusInitVolt().equals(
				AcscCaseData.ScBusVolt_UnitVolt) ? ScBusVoltage.UNIT_VOLT
	...
	public static boolean dstabCaseData2AlgoMapping(DStabCaseData dstabData,
		...
		algo
				.setSimuMethod(dstabData.getSimuMethod().equals(
						...
						: DynamicSimuMethods.MODIFIED_EULER);
		...
		if (dstabData.isAbsoluteMachValue()) {
			...
			Machine mach = algo.getDStabNet().getMachine(
					dstabData.getRefMachId());
	 ...
	 * @param msg
	 */
	private static boolean dstabCaseData2NetMapping(DStabCaseData dstabData,
		...
		dstabNet.setStaticLoadModel(dstabData.getStaticLoadType().equals(
				DStabCaseData.StaticLoad_Const_Z) ? StaticLoadModel.CONST_Z
		...
		if (dstabData.getDisableDynamicEvent()) {
			if (dstabData.isSetPointChange()) {
				...
				eSetPoint
						.setControllerType(dstabData.getSelectedController()
								.equals(Constants.Token_Exciter) ? ControllerType.EXCITER
								: dstabData.getSelectedController().equals(
										Constants.Token_Governor) ? ControllerType.GOVERNOR
										: ControllerType.STABILIZER);
				eSetPoint.setChangeValue(dstabData.getSetPointValueChange());
				eSetPoint.setAbusoluteChange(dstabData
						.isSetPointChangeAbsolute());
				event.setBusDynamicEvent(eSetPoint);
			}
		} else {
			for (int i = 0; i < dstabData.getDEventList().size(); i++) {
				DStabDEventData eventData = (DStabDEventData) dstabData
						.getDEventList().get(i);
				// make sure that event name is not "" or NewEventName
				if (!eventData.getEventName().equals(
						DStabDEventData.NewEventName)
						&& !eventData.getEventName().trim().equals("")) {
					IpssLogger.getLogger().info("Event Data: " + eventData);
					// create event name
					String name = "EventAt_" + eventData.getStartTime()
							+ eventData.getType();
					// map event type
					DynamicEventType deType = CaseData2AlgorithmMapperImpl
							.getDEventType(eventData.getType());
					// create the DStabEvent
					DynamicEvent event = DStabObjectFactory.createDEvent(
							eventData.getEventName(), name, deType, dstabNet,
							msg);
					if (event == null) {
					...
					try {
						CaseData2AlgorithmMapperImpl.setEventData(event,
								eventData, dstabData.getTotalSimuTime(),
								dstabNet, msg);
					} catch (Exception e) {
						IpssLogger.logErr(e);
						SpringAppContext.getEditorDialogUtil()
	...
	private static void acscFaultData2AcscBusFaultMapping(AcscFaultData data,
			AcscBusFault fault) {
		fault
				.setFaultCode(data.getCategory().equals(
						AcscFaultData.FaultCaty_Fault_LLG) ? SimpleFaultCode.GROUND_LLG
						: (data.getCategory().equals(
								AcscFaultData.FaultCaty_Fault_LG) ? SimpleFaultCode.GROUND_LG
								: (data.getCategory().equals(
										AcscFaultData.FaultCaty_Fault_LL) ? SimpleFaultCode.GROUND_LL
	...
	private static void acscFaultData2AcscBranchFaultMapping(
			AcscFaultData data, AcscBranchFault fault) {
		fault
				.setFaultCode(data.getCategory().equals(
						AcscFaultData.FaultCaty_Fault_LLG) ? SimpleFaultCode.GROUND_LLG
						: (data.getCategory().equals(
	...
	private static DynamicEventType getDEventType(String eventDataType) {
		if (eventDataType.equals(DStabDEventData.DEventType_BusFault))
		...
		else if (eventDataType
				.equals(DStabDEventData.DEventType_SetPointChange))
			...
			throw new InvalidParameterException(
					"Programming error, eventDataType: " + eventDataType);
	}

	private static void setEventData(DynamicEvent event,
			DStabDEventData eventData, double toltalSimuTime,
		...
		if (eventData.getType().equals(DStabDEventData.DEventType_LoadChange)) {
			...
			if (eventData.getLoadChangeData().getChangeType().equals(
					DStabLoadChangeData.FixedTime))
				eventData.setStartTime(eventData.getLoadChangeData()
						.getThreshhold());
			else
		...
		if (eventData.getType().equals(DStabDEventData.DEventType_LoadChange)) {
			...
			eLoad
					.setType(ldata.getChangeType().equals(
							DStabLoadChangeData.LowFreq) ? LoadChangeEventType.LOW_FREQUENCY
							: (ldata.getChangeType().equals(
									DStabLoadChangeData.LowVolt) ? LoadChangeEventType.LOW_VOLTAGE
		...
		} else if (eventData.getType().equals(
				DStabDEventData.DEventType_BranchOutage)) {
			...
			event
					.setType(eventData.getType().equals(
							DStabDEventData.DEventType_BusFault) ? DynamicEventType.BUS_FAULT
							: DynamicEventType.BRANCH_FAULT);
			...
			if (fdata.getType().equals(AcscFaultData.FaultType_BusFault)) {
				AcscBusFault fault = CoreObjectFactory
						.createAcscBusFault(Constants.Token_BusFaultId
								+ fdata.getBusId());
				CaseData2AlgorithmMapperImpl.acscFaultData2AcscBusFaultMapping(
						fdata, fault);
				...
				DStabBranchFault fault = CaseData2AlgorithmMapperImpl
				...
				if (fault.isReclosure()) {
					...
					event2.setBranchFault(CaseData2AlgorithmMapperImpl
							.createDStabBranchFault(fdata, dstabNet));
				}
			}
		}
	...
	private static DStabBranchFault createDStabBranchFault(AcscFaultData fdata,
			DStabilityNetwork dstabNet) {
