...
import org.interpss.mapper.ieee_odm.ODMXmlHelper;
...
public class ODM_Old_DataMapperImpl {
	/**
	...
	public static AclfBus mapBusData(BusRecordXmlType busRec, AclfAdjNetwork adjNet) throws Exception {
		...
		if (busRec.getLoadflowData() != null) {
			ODM_Old_DataMapperImpl.setBusLoadflowData(busRec.getLoadflowData(), aclfBus, adjNet);
		}
		return aclfBus;
	}
	...
	public static AclfBranch mapBranchData(BranchRecordXmlType branchRec, AclfAdjNetwork adjNet, IPSSMsgHub msg) throws Exception {
		...
		if (branchRec.getLoadflowData().size() > 0) {
			if (branchRec.getLoadflowData().size() == 1)
				ODM_Old_DataMapperImpl.setBranchLoadflowData( 
