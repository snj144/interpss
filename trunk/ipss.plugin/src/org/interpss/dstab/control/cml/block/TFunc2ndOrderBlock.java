 /*
  * @(#)TFunc2ndOrderBlock.java   
  *
  * Copyright (C) 2006 www.interpss.org
  *
  * This program is free software; you can redistribute it and/or
  * modify it under the terms of the GNU LESSER GENERAL PUBLIC LICENSE
  * as published by the Free Software Foundation; either version 2.1
  * of the License, or (at your option) any later version.
  *
  * This program is distributed in the hope that it will be useful,
  * but WITHOUT ANY WARRANTY; without even the implied warranty of
  * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  * GNU General Public License for more details.
  *
  * @Author Mike Zhou
  * @Version 1.0
  * @Date 10/30/2006
  * 
  *   Revision History
  *   ================
  *
  */

package org.interpss.dstab.control.cml.block;

import com.interpss.common.datatype.LimitType;
import com.interpss.common.util.IpssLogger;
import com.interpss.dstab.controller.block.ControlBlock1stOrderAdapter;
import com.interpss.dstab.controller.block.IStaticBlock;

/**
 * Transfer Function:
 *  
 *       y = [ k / ( 1 + a s + b s^2 ) ] u 
 *       
 * Diff eqn      
 *       dot(x1) = |  0    1   | |x1| + | 0 | u
 *       dot(x2)   | -1/b -a/b | |x2|   |k/b|
 *       
 *        y = [ 1  0 ] |x1| = x1
 *                     |x2|
 *        x2(0) = 0
 *        x1(0) = k * u(0);
 *        y(0)  = k * u(0);             
 *                      
 * @author mzhou
 *
 */

public class TFunc2ndOrderBlock extends ControlBlock1stOrderAdapter {
	private double k = 0.0;
	private double a = 0.0;
	private double b = 0.0;
	private LimitType limit = null;
	
	/*
		u  :     protected double StaticBlockAdapter.u = 0.0;
		x1 :     protected double ControlBlockAdapter.stateX = 0.0;   
	 	dx1/dt : protected double ControlBlock1stOrderAdapter.dX_dt = 0.0;
    */
	private double stateX2 = 0.0;   
 	private double dX2_dt = 0.0;
	
	public TFunc2ndOrderBlock(double k, double a, double b) {
		setType(IStaticBlock.Type.NoLimit);
		this.k = k;
		this.a = a;
		this.b = b;
	}
	
	public TFunc2ndOrderBlock(double k, double a, double b, double max, double min) {
		this(k, a, b);
		setType(IStaticBlock.Type.Limit);
		limit = new LimitType(max, min);
	}

	public boolean initStateY0(double y0) {
		 //        x2(0) = 0
		 //        x1(0) = k * u(0) = y0;
		if ( k <= 0.0 ) {
			IpssLogger.getLogger().severe("TFunc2ndOrderBlock.initState(), k <= 0.0");
			return false;
		}
		else if ( b < 0.0 ) {
			IpssLogger.getLogger().severe("TFunc2ndOrderBlock.initState() b < 0.0");
			return false;
		}
		else if ( b == 0.0 && a != 0) {
			IpssLogger.getLogger().severe("TFunc2ndOrderBlock.initState() b == 0.0 && a != 0");
			return false;
		}
		else if (limit != null && limit.isViolated(y0)) {
			IpssLogger.getLogger().severe("TFunc2ndOrderBlock.initState() limit violation");
			return false;
		}
		
		setU(y0/k);
		stateX = 0.0;
		stateX2 = y0;
		dX_dt = 0.0;
		dX2_dt = 0.0;
		return true;
	}
	
	public boolean initStateU0(double u0) {
		setU(u0);
		double y0 = k * u0;             
		return initStateY0(y0);
	}

	public double getU0() {
		return getU();
	}
	
	public void eulerStep1(double u, double dt) {
		this.u = u;
		this.dX_dt = dX_dt(u);
		this.dX2_dt = dX2_dt(u);
		this.stateX += dX_dt * dt;
		this.stateX2 += dX2_dt * dt;
	}
	
	public void eulerStep2(double u, double dt) {
		this.u = u;
		stateX += 0.5 * (dX_dt(u) - this.dX_dt) * dt;	
		stateX2 += 0.5 * (dX2_dt(u) - this.dX2_dt) * dt;	
	}
	
	/*
 *       dot(x1) = |  0    1   | |x1| + | 0 | u
 *       dot(x2)   | -1/b -a/b | |x2|   |k/b|
	 */

	protected double dX_dt(double u) {
		if (b > 0.0)
			return stateX2;
		else
			return 0.0;
	}
	
	protected double dX2_dt(double u) {
		if (b > 0.0)
			return ( -stateX - a*stateX2 + k*u) / b;
		else
			return 0.0;
	}	
	
	/*
     *        y = [ 1  0 ] |x1| = x1
     *                     |x2|
	 */
	public double getY() {
		double y = stateX;
		if (b == 0.0)
			y = k * getU();
		if (type == IStaticBlock.Type.Limit) {
			y = limit.limit(y);
		}
		return y;
	}

	public String toString() {
		String str = "type, k, a, b: " + getType() 
		         + ", " + k + ", " + a + ", " + b;
		return str;
	}	
}
