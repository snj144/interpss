...
import org.interpss.editor.util.ImageIconBean;
import org.interpss.editor.util.ShadowBorder;
...
public class IpssGraphCodec {

	private static IpssGraphCodec _instance;
	 ...
	 * 
	 * @see org.interpss.editor.coreframework.GPGraphpad#initFileFormat() method
	 *      instead.
	 * @return the singleton instance
	 */
	public static IpssGraphCodec getInstance(GPGraphpad pad) {
		if (null == _instance) {
			_instance = new IpssGraphCodec(pad);
		}
		return _instance;
	}
	 ...
	 *      instead.
	 */
	protected IpssGraphCodec(GPGraphpad pad) {
	...
	public GraphModel read(InputStream in, JGraph graph) throws Exception {
		...
		DefaultGraphCell[] insert = parseChildren(model, modelNode, cells, cs);
	...
	public DefaultGraphCell parseCell(GraphModel model, Node node,
		...
		if (node.getNodeName().toLowerCase().equals("a")) {
			...
			if (key != null && type != null) {
				...
				cell = createCell(model, type.getNodeValue(), userObject);
				// attribute map

				if (cell != null) {
					cells.put(key.getNodeValue(), cell);

					DefaultGraphCell[] children = parseChildren(model, node,
							cells, cs);
	...
	public DefaultGraphCell[] parseChildren(GraphModel model, Node node,
			Hashtable cells, ConnectionSet cs) {
		List list = new LinkedList();
		for (int i = 0; i < node.getChildNodes().getLength(); i++) {
			Node child = node.getChildNodes().item(i);
			DefaultGraphCell cell = parseCell(model, child, cells, cs);
			if (cell != null)
				list.add(cell);
		}
	...
	public String toString(JGraph graph, GPDocument doc) {
		...
		xml.append("<model>\n");
		xml.append(outputModel(model, "\t", null));
		xml.append("</model>\n");

		xml.append("<attrs>\n");
		xml.append(outputAttributes("\t"));
		xml.append("</attrs>\n");

		xml.append("<user>\n");
		xml.append(encodeUserObjects("\t", userObjectMap));
		xml.append("</user>\n");
	...
	public static DefaultGraphCell createCell(GraphModel model, String type,
		...
		if (type.equals(PORT))
			...
			// userObject = new UserObject((Map) userObject);
			System.out
					.println("Programming error, userObject is an instance of Map");
		}
		return null;
