...
public class CloseTabPaneUI extends BasicTabbedPaneUI {
	...
	private BufferedImage closeAImgB;

	private BufferedImage maxAImgB;

	private BufferedImage closeAImgI;

	private BufferedImage maxAImgI;
	...
	public CloseTabPaneUI() {
		...
		maxItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				((CloseAndMaxTabbedPane) tabPane).fireMaxTabEvent(null, tabPane
						.getSelectedIndex());
			}
		});
		...
		closeItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				((CloseAndMaxTabbedPane) tabPane).fireCloseTabEvent(null,
	...
	private void initIcons() {
		...
		getCloseIcon().paintIcon(closeB, closeImgI.createGraphics(), 0, 0);

		getMaxIcon().paintIcon(maxB, maxImgI.createGraphics(), 0, 0);

		closeAImgB = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
				BufferedImage.TYPE_4BYTE_ABGR);

		maxAImgB = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
				BufferedImage.TYPE_4BYTE_ABGR);

		closeAImgI = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
				BufferedImage.TYPE_4BYTE_ABGR);

		maxAImgI = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
				BufferedImage.TYPE_4BYTE_ABGR);

		getCloseOverIcon().paintIcon(closeB, closeAImgI.createGraphics(), 0, 0);

		getMaxOverIcon().paintIcon(maxB, maxAImgI.createGraphics(), 0, 0);

	}

	protected Icon getCloseIcon() {
		return WindowsIconFactory.createFrameCloseIcon();
	}

	protected Icon getMaxIcon() {
		return WindowsIconFactory.createFrameMaximizeIcon();
	}
	...
	protected void paintCloseIcon(Graphics g, int dx, int dy, boolean isOver) {
		if (!isOver) {
			...
			paintActionButton(g, dx, dy, closeIndexStatus, isOver, closeB,
					closeAImgB);
			g.drawImage(closeAImgI, dx, dy + 1, null);
		}
	}
	...
	protected void paintMaxIcon(Graphics g, int dx, int dy, boolean isOver) {
		...
		if (!isOver) {
			...
			paintActionButton(g, dx, dy, maxIndexStatus, isOver, maxB, maxAImgB);
			g.drawImage(maxAImgI, dx, dy + 1, null);
		}
	...
	class MyMouseHandler extends MouseHandler {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getClickCount() > 1 && overTabIndex != -1) {
				((CloseAndMaxTabbedPane) tabPane).fireDoubleClickTabEvent(e,
						overTabIndex);
			}
		}
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (maxIndexStatus == PRESSED) {
				maxIndexStatus = OVER;
				tabScroller.tabPanel.repaint();
				((CloseAndMaxTabbedPane) tabPane).fireMaxTabEvent(e,
						overTabIndex);
				return;
			}
