...
//		  31 BUS-31  100   1  1  3 0.982 0.     9.2      4.6       572.8349207.0362 100.    .98200 999900 -99990    0.      0.        0                                                                                                                                                                            
	...
	public void bus30testCase() throws Exception{
		...
		AclfNetwork net = simuCtx.getAclfNet();
 		...
 		AclfBus swingBus = (AclfBus)net.getBus("Bus1");
		SwingBusAdapter swing = (SwingBusAdapter)swingBus.getAdapter(SwingBusAdapter.class);
  		assertTrue( Math.abs(swing.getGenResults(Type.PU).getReal()-2.6095)<0.0001);
  		assertTrue( Math.abs(swing.getGenResults(Type.PU).getImaginary()+0.1653)<0.0001);
	...
	public void bus57testCase() throws Exception{
		...
		//System.out.println(swing.getGenResults(UnitType.PU, net.getBaseKva()).im);
  		assertTrue(Math.abs(swing.getGenResults(Type.PU).getReal()-4.7942)<0.0001);
  		assertTrue(Math.abs(swing.getGenResults(Type.PU).getImaginary()-1.2951)<0.0001);
	...
	public void bus118testCase() throws Exception{
		...
		SwingBusAdapter swing = (SwingBusAdapter)swingBus.getAdapter(SwingBusAdapter.class);
		System.out.println(swing.getGenResults(Type.PU).getReal());
		System.out.println(swing.getGenResults(Type.PU).getImaginary());
  		assertTrue(Math.abs(swing.getGenResults(Type.PU).getReal()-5.13442)<0.0001);
  		assertTrue(Math.abs(swing.getGenResults(Type.PU).getImaginary()+0.82383)<0.0001);
	}
}
