...
public class IpssNewTextDialog extends javax.swing.JDialog {
	...
	protected void initGuiComponents() {
		...
		selectpanel.setBorder(new TitledBorder(null, "Contents",
		...
		newRadioButton.setText("Create a New Text File");
		...
		fromRadioButton.setText("Import a Text File from existing source");
		...
		nameLabel.setText("Text File Name:");
	...
	public void setFile(IpssTextFile file) {
		this.file = file;
	}

	public IpssTextFile getFile() {
		return file;
	}
	...
	public void updateState() {
		if (nameTextField.getText().equals("")) {
			showError(Translator.getString("TextDialog.Name.Error"));
			return;
		}
		...
		try {
			...
			if (myFile.exists()) {
				...
				showError(Translator.getString("TextDialog.File.Exists"));
				return;
			}
		...
		showMessage(Translator.getString("TextDialog.OK"));
		// showMessage(filePathName);
	}

	public void selectFile() {
		try {
			NamedInputStream in = com.interpss.editor.util.Utilities
					.provideInput(Translator.getString("TextFileExtension"),
							Translator
									.getString("TextFileExtensionDescription"));
			if (in != null) {
				setFile(com.interpss.editor.util.Utilities.OpenTextFile(
						graphpad, in.getName()));
				this.setSrcFileName(in.getName());
				dirTextField.setText(in.getName());
				if (getFile() == null) {
					showError("Can't open the selected text file.");
					return;
				}
			}
		} catch (Exception ex) {
