...
public class Type3_psatAvr extends AbstractExciter {
	...
	private static final NBType3_psatAvrEditPanel _editPanel = new NBType3_psatAvrEditPanel();
	
	/**
	 * Default Constructor
	 *
	 */
	public Type3_psatAvr() {
		this("excId", "PSAT_AVR_TYPE3"); 
	}
	 ...
	 */
	public Type3_psatAvrData getData() {
		return (Type3_psatAvrData)_data;
	}
	...
	public void setDataXmlString(final String xmlString) {
		...
		_data = XmlUtil.toObject(xmlString, Type3_psatAvrData.class);
	}
	...
	public void initStates(final IPSSMsgHub msg) {
		limit = new LimitType(getData().getVfmax(), getData().getVfmin()); 
...
//		return ( getData().getKa() * ( vref + vpss - vt ) - X1) / getData().getTa();
	...
	public Hashtable getStates(Object ref) {
		...
		table.put(DStabOutFunc.OUT_SYMBOL_EXC_EFD, new Double(limit.limit(x1)));
		return table;
	}
