...
public class FilterControlBlock extends ControlBlock1stOrderAdapter {
	...
	public FilterControlBlock(Type type, double k, double t1, double t2, double max, double min) {
		this(k, t1, t2);
		setType(type);
	...
	public boolean initStateY0(double y0) {
		if ( getK() <= 0.0 ) {
			IpssLogger.getLogger().severe("FilterControlBlock.initState(), k <= 0.0");
			return false;
		}
		setU(y0/getK());
	...
	protected double dX_dt(double u) {
		if (getT2() > 0.0)
			return (getK() * (1.0 - getT1()/getT2()) * u - getStateX()) / getT2();
		else 
			return 0.0;
	}
	...
	private boolean isLimitViolated(double u) {
		return (limit.getMin()-getU1(u)) > getStateX() || getStateX() > (limit.getMax()-getU1(u));
	}
	...
	public String toString() {
		String str = "type, k, t1, t2, limit: " + getType() + ", "  + k  + ", "  +  t1 +  ", "   +  t2 +  ", " + limit;
		return str;
	}	
}
