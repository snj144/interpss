...
import com.interpss.editor.util.IEdgeFactory;
import com.interpss.editor.util.IVertexFactory;
...
public class GPMarqueeHandler extends BasicMarqueeHandler {
    ...
    public void setDocument(GPDocument document) {
        ...
        buttonSelect = (JToggleButton) ((IpssAbstractGraphAction) document.getCommand("ToolBoxSelect")).getToolComponent(null);
        buttonZoomArea = (JToggleButton) ((IpssAbstractGraphAction) document.getCommand("ToolBoxZoomArea")).getToolComponent(null);
...
//			else if (isEdgeToolBoxSelected() >= 0) {
			else if (event != null && !event.isConsumed() && port != null && firstPort != null	&& firstPort != port) {
				((IEdgeFactory) document.getEdgeCreators().get(isEdgeToolBoxSelected())).addEdge(start, current, firstPort, port);
				if (!remanent)
				{
					buttonSelect.doClick();
				}
