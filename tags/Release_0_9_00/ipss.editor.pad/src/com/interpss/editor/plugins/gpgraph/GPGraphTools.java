...
public class GPGraphTools {
	...
	private static GPGraphTools instance;
	
	public static GPGraphTools getInstance() {
		if (instance == null)
			instance = new GPGraphTools();
		return instance;
	}
	
	private GPGraphTools() {
	}
	...
	*/
	public Object[] getSpanningTree(GPGraph graph, CostFunction cf) {
		...
		while (!edges.isEmpty()) {
			...
			edges.remove(edge);
			Object setA, setB;
			setA = uf.find(graph.getModel().getSource(edge));
			setB = uf.find(graph.getModel().getTarget(edge));
