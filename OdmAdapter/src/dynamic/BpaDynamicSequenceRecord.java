...
public class BpaDynamicSequenceRecord {
	...
	public String setDynamicSequenceData(String bpaLfNetStr, DStabBusXmlType DStabBus) {
		...
		//R0
		double R0=busXml.getScShuntLoadData().getZeroZ().getRe();
		String R0Str=StringUtil.format(R0, 7, 4);
		//X0
		double X0=busXml.getScShuntLoadData().getZeroZ().getIm();
		String X0Str=StringUtil.format(X0, 7, 4);
	...
	public String setDynamicMutualZeroData(String bpaLfNetStr, LineDStabXmlType line) {
		for(MutualZeroZXmlType mutualZero:line.getLineMutualZeroZ()){
			...
			DStabBusXmlType MLFromBus=(DStabBusXmlType)mutualZero.getMutualBranchFromBus().getIdRef();
			...
			DStabBusXmlType MLToBus=(DStabBusXmlType)mutualZero.getMutualBranchToBus().getIdRef();
			...
			//PAR2
			String CirId2=(mutualZero.getMutualBranchCirId()=="1")?" ":mutualZero.getMutualBranchCirId();
			//RM
			...
			try {
				LineDStabXmlType mutualBranch =parser.getDStabLine
					(MLFromBus.getId(),MLToBus.getId(),mutualZero.getMutualBranchCirId());
				for(MutualZeroZXmlType mutualZ0Line2:mutualBranch.getLineMutualZeroZ()){
					if(mutualZ0Line2.getMutualBranchFromBus().getIdRef()==braXml.getFromBus().getIdRef()
						&&mutualZ0Line2.getMutualBranchToBus().getIdRef()==braXml.getToBus().getIdRef()
						&&mutualZ0Line2.getMutualBranchCirId()==braXml.getCircuitId()){
