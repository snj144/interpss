...
import org.interpss.odm.converter.BPA.dynamic.BpaDynamicGeneratorRecord;
import org.interpss.odm.converter.BPA.dynamic.BpaDynamicPSSRecord;
import org.interpss.odm.converter.BPA.dynamic.BpaDynamicTurbineGovernorRecord;
		...
		for(int i=0;i<size;i++){
			...
			if(DStabBus.getDynamicGen()!=null){
				for(DynamicGeneratorXmlType dynGen:DStabBus.getDynamicGen()){
					...
					bpaGenRecord.setDynamicGeneratorData(bpa, DStabBus, dynGen);
					System.out.println(bpaGenRecord.stdout());
					...
					if(dynGen.getGovernor()!=null){
						...
						bpaGovRecord.setDynamicTurbineGovernorData(bpa, DStabBus, dynGen);
						System.out.println(bpaGovRecord.stdout());
					}
				}
			}
	    }
