...
public class XmlNetParamModifier {
	private static enum ParamType {Power, Z};
	 ...
	 */
	public static boolean applyModification2Net(Network net, ModificationXmlType mod) {
		// apply the Network-level changes
		if (mod.getNetChangeRec() != null) {
			if (mod.getNetChangeRec().getBusChangeRecArray().length > 0) {
				for (BusChangeRecXmlType busRec : mod.getNetChangeRec().getBusChangeRecArray()) {
  			...
  			if (mod.getNetChangeRec().getBranchChangeRecArray().length > 0) {
  				for (BranchChangeRecXmlType braRec : mod.getNetChangeRec().getBranchChangeRecArray()) {
		...
		// apply the AclfNetwork-level changes
  		if (mod.getAclfNetChangeRec() != null && net instanceof AclfNetwork) {
			...
			if (mod.getAclfNetChangeRec().getAclfBusChangeRecArray().length > 0) {
				for (AclfBusChangeRecXmlType busRec : mod.getAclfNetChangeRec().getAclfBusChangeRecArray()) {
					AclfBus bus = (AclfBus)getBus(busRec, aclfNet);
					if (bus != null) {
						if (busRec.getLoadChange() != null) {
							// modify load value
							if (busRec.getLoadChange().getValueChange() != null) {
								Complex x = applyPowerChangeRec(bus.getLoad(), busRec.getLoadChange().getValueChange(), aclfNet.getBaseKva());
							...
							if (busRec.getLoadChange().getCodeChange() != null) {
								AclfLoadCodeChangeXmlType codeChange = busRec.getLoadChange().getCodeChange(); 
								bus.setLoadCode(
									codeChange.getCode() == AclfLoadCodeXmlData.CONST_P? AclfLoadCode.CONST_P :
										(codeChange.getCode() == AclfLoadCodeXmlData.CONST_I? AclfLoadCode.CONST_I :
											(codeChange.getCode() == AclfLoadCodeXmlData.CONST_Z? AclfLoadCode.CONST_Z :
												(codeChange.getCode() == AclfLoadCodeXmlData.EXPONENTIAL? AclfLoadCode.EXPONENTIAL : AclfLoadCode.NON_LOAD))));
			...
			if (mod.getAclfNetChangeRec().getAclfBranchChangeRecArray().length > 0) {
				for (AclfBranchChangeRecXmlType braRec : mod.getAclfNetChangeRec().getAclfBranchChangeRecArray()) {
					AclfBranch branch = (AclfBranch)getBranch(braRec, aclfNet);
					if (branch != null) {
						if (braRec.getZChange() != null) {
							Complex z = applyComplexParamChangeRec(branch.getZ(), braRec.getZChange(), 
											            ParamType.Z, aclfNet.getBaseKva(), branch.getHigherBaseVoltage());
							branch.setZ(z);
						}
					} else {
			  			return false;
	...
	private static Complex applyPowerChangeRec(Complex original, ComplexParamChangeXmlType changeRec, double baseKva) {
		return applyComplexParamChangeRec(original, changeRec, ParamType.Power, baseKva, 1.0);
	}
	
	private static Complex applyComplexParamChangeRec(Complex original, ComplexParamChangeXmlType changeRec, ParamType ptype, double baseKva, double busBaseVolt) {
		if (changeRec.getAction() == ChangeActionXmlData.ADD ||
			...
			if (changeRec.getUnit() != UnitXmlData.PU) {
				...
				byte unit = IpssXmlAdapter.mapXmlUnitType2IpssUnitType(changeRec.getUnit());
				if (ptype == ParamType.Power) {
					...
					Complex z = UnitType.zConversion(new Complex(re,im), busBaseVolt, baseKva, unit, UnitType.PU);
					re = z.getReal();
					im = z.getImaginary();
				}
			}
			...
			if (changeRec.getAction() == ChangeActionXmlData.DECREASE)
				factor = -factor;
			return new Complex(original.getReal()*(1.0+factor), original.getImaginary()*(1.0+factor));
		}
