...
public static String b11MatrixToString( AclfNetwork net ) {
	try {
		...
		for( Bus b : net.getBusList()) {
			...
			str += "bus: " + bus.getId() + "    " +
			       "bii: " + Number2String.toStr(bus.b11ii()) + "\n";
			for ( Object br : bus.getBranchList() ) {
				AclfBranch bra = (AclfBranch)br;
				str += "          " + bra.getId() + "   " +
			           "bij: " + Number2String.toStr(bra.b11ft()) + "\n";
			}
		}
	  return str + "\n";
...
public static String b1MatrixToString( AclfNetwork net ) {
		...
		for( Bus b : net.getBusList()) {
			AclfBus bus = (AclfBus)b;
			str += "bus: " + bus.getId() + "    " +
			       "bii: " + Number2String.toStr(bus.b1ii()) + "\n";
			for (Branch br : bus.getBranchList() ) {
				AclfBranch bra = (AclfBranch)br;
				str += "          " + bra.getId() + "   " +
			           "bij: " + Number2String.toStr(bra.b1ft()) + "\n";
			}
		}
	  return str + "\n";
}
...
public static String lfResultsBusStyle( AclfNetwork net ) {
	...
	try {
		...
		str.append( "\n\n                                              Load Flow Results\n\n");
		str.append( "-------------------------------------------------------------------------------------------------------------------------------------\n" );
		str.append( " Bus ID             Bus Voltage         Generation           Load             To             Branch P+jQ          Xfr Ratio   PS-Xfr\n" );
		str.append( "               base     Mag   Ang     (mW)    (mVar)    (mW)    (mVar)      Bus ID      (mW)    (mVar)   (kA)   (From)  (To)   Angle\n" );
		str.append( "-------------------------------------------------------------------------------------------------------------------------------------\n" );
			...
			for ( Branch br : bus.getBranchList()) {
				...
				if (bra.isXfr() || bra.isPSXfr()) {
					...
					if (bra.isPSXfr()) {
						...
						str.append( "   " + Number2String.toStr( "##0.0", psXfr.getFromAngle(UnitType.Deg)));
					}	
					str.append("\n");
				}
				else {
	...
	StringBuffer str = new StringBuffer("");
  	try {
		...
		for( Bus b : net.getBusList()) {
			...
			str.append( Number2String.toStr("###0.00000",bus.getVoltageMag(UnitType.PU)) + " ");
			str.append( Number2String.toStr("######0.0",bus.getVoltageAng(UnitType.Deg)) + "   ");
...
public static String maxMismatchToString( AclfNetwork net ) {
	try {
	  double baseKVA = net.getBaseKva();
		...
		str += Number2String.toStr( "####0.000000",mis.maxMis.getImaginary()) + " (pu)\n";
		str += Number2String.toStr(21, " ");
		str += Number2String.toStr( "####0.000000", baseKVA*mis.maxMis.getReal()) + " ";
		str += Number2String.toStr(12, " ");
		str += Number2String.toStr("####0.000000",baseKVA*mis.maxMis.getImaginary()) + " (kva)\n";
		return str;
	} catch (Exception emsg) {
		return emsg.toString();
	}
...
public static String branchMvaRatingViolationList(AclfNetwork net ) {
    ...
    if (net.hasBranchMavRatingViolation()) {
    	str.append("\n\n");
    	str.append( "                        Branch MvaRating Violation List\n\n" );
    	str.append( "         BranchID        MvaFlow      Side    MvaRating1   MvaRating2   MvaRating3\n" );
    	str.append( "     ----------------- ------------ -------- ------------ ------------ ------------\n" );
		for( Branch b : net.getBranchList()) {
			...
			if (bra.isMvaRatingViolated(net.getBaseKva())) {
				...
				double mva = bra.powerFrom2To(UnitType.mVA, net.getBaseKva()).abs();
				String side = "From";
				if (bra.powerFrom2To(UnitType.mVA, net.getBaseKva()).abs() < bra.powerTo2From(UnitType.mVA, net.getBaseKva()).abs() ) {
					mva = bra.powerTo2From(UnitType.mVA, net.getBaseKva()).abs();
					side = "To";
				}
...
public static String pvBusLimitToString(AclfAdjNetwork net) throws Exception {
	StringBuffer str = new StringBuffer("");
	...
	str.append( "      BusID     Vact     Vspec      Q      Qmax     Qmin   Status\n" );
	str.append( "     -------- -------- -------- -------- -------- -------- ------\n" );
	...
	for( PVBusLimit pv : net.getPvBusLimitList()) {
		...
		str.append( Number2String.toStr( "###0.0000", pv.getAclfBus().getVoltageMag(UnitType.PU) ) );
		str.append( Number2String.toStr( "###0.0000", pv.getVSpecified(UnitType.PU) ) );
		str.append( Number2String.toStr( "#####0.00", genBus.getGenResults(UnitType.PU,baseKVA).getImaginary() ) );
		str.append( Number2String.toStr( "#####0.00", pv.getQLimit(UnitType.PU,baseKVA).getMax() ) );
		str.append( Number2String.toStr( "#####0.00", pv.getQLimit(UnitType.PU,baseKVA).getMin() ) );
		str.append( Number2String.toStr(6, pv.isActive() ? "on" : "off" ) + "\n");
	}
	return str.toString();
}

public static String pqBusLimitToString(AclfAdjNetwork net) throws Exception {
	StringBuffer str = new StringBuffer("");
	...
	str.append( "      BusID     Qact     Qspec      V      Vmax     Vmin   Status\n" );
	str.append( "     -------- -------- -------- -------- -------- -------- ------\n" );
	...
	for( PQBusLimit pq : net.getPqBusLimitList()) {
		...
		str.append( Number2String.toStr("####0.00",genBus.getGenResults(UnitType.PU,baseKVA).getImaginary()) + " " );
		str.append( Number2String.toStr("####0.00",pq.getQSpecified(UnitType.PU,baseKVA)) + " " );
		str.append( Number2String.toStr("##0.0000",pq.getAclfBus().getVoltageMag(UnitType.PU)) + " " );
		str.append( Number2String.toStr("##0.0000",pq.getVLimit(UnitType.PU).getMax()) + " " );
		str.append( Number2String.toStr("##0.0000",pq.getVLimit(UnitType.PU).getMin()) + " " );
		str.append( Number2String.toStr(5, pq.isActive() ? "on" : "off" ) + "\n");
	}
	return str.toString();
}

public static String remoteQBusToString(AclfAdjNetwork net) throws Exception {
	StringBuffer str = new StringBuffer("");
	...
	str.append( "       VcBus    Type    ReQBus/Branch   Actual    Spec       Q      Qmax     Qmin   Status\n" );
	str.append( "     -------- -------- --------------- -------- -------- -------- -------- -------- ------\n" );
	...
	for( RemoteQBus re : net.getRemoteQBusList()) {
		...
		str.append( Number2String.toStr(-9, (re.getControlType()==RemoteQControlType.BUS_VOLTAGE?" Voltage":"MvarFlow")));
		str.append( Number2String.toStr(15, re.getControlType()==RemoteQControlType.BUS_VOLTAGE?
									re.getRemoteBus().getId():re.getRemoteBranch().getId()));
		str.append( Number2String.toStr( "###0.0000", re.getControlType()==RemoteQControlType.BUS_VOLTAGE?
		...
		str.append( Number2String.toStr( "#####0.00", genBus.getGenResults(UnitType.PU,baseKVA).getImaginary()) );
		str.append( Number2String.toStr( "#####0.00", re.getQLimit(UnitType.PU,baseKVA).getMax() ) );
		str.append( Number2String.toStr( "#####0.00", re.getQLimit(UnitType.PU,baseKVA).getMin() ) );
		str.append( Number2String.toStr(6, re.isActive() ? "on" : "off" ) + "\n");
	}
	return str.toString();
}

public static String aclfFuncLoadToString(AclfAdjNetwork net) throws Exception {
	StringBuffer str = new StringBuffer("");
	...
	str.append( "      BusID     Pact     Qact       V      P(0)     Q(0)   Status\n" );
	str.append( "     -------- -------- -------- -------- -------- -------- ------\n" );
	...
	for( FunctionLoad x : net.getFunctionLoadList()) {
		...
		str.append( Number2String.toStr("##0.0000",x.getP().getLoad(vpu,UnitType.PU,baseKVA)) + " " );
		str.append( Number2String.toStr("##0.0000",x.getQ().getLoad(vpu,UnitType.PU,baseKVA)) + " " );
		str.append( Number2String.toStr("##0.0000",x.getAclfBus().getVoltageMag(UnitType.PU)) + " " );
		str.append( Number2String.toStr("##0.0000",x.getP().getLoad0(UnitType.PU,baseKVA)) + " " );
		str.append( Number2String.toStr("##0.0000",x.getQ().getLoad0(UnitType.PU,baseKVA)) + " " );
		str.append( Number2String.toStr(5, x.isActive() ? "on" : "off" ) + "\n");
	}
	return str.toString();
}

public static String tapVControlToString(AclfAdjNetwork net) throws Exception {
	StringBuffer str = new StringBuffer("");

	double baseKva = net.getBaseKva();

	str.append("\n\n");
	str.append( "                          Tap Voltage Adjustment/Control\n\n" );
	str.append( "          BranchID     VC BusID  Actual   Spec/Range     Tap  Tmax  Tmin  StepSize Status\n" );
	str.append( "     ----------------- -------- -------- -------------- ----- ----- -----   -----  ------\n" );
	...
	for( TapControl x : net.getTapControlList()) {
		...
		if (x.getControlType() == XfrTapControlType.BUS_VOLTAGE) {
			...
			str.append( Number2String.toStr("##0.0000", x.getVcBus().getVoltageMag(UnitType.PU)) + " " );
			if (x.getFlowControlType() == FlowControlType.POINT_CONTROL)
				str.append( Number2String.toStr("##0.0000", x.getVSpecified(UnitType.PU)) + " " );
			else
				str.append( x.getControlRange() + " " );
		}
		else {
			str.append( Number2String.toStr(-8, " "));
			str.append( Number2String.toStr("##0.0000", x.getMvarFlowCalculated(UnitType.PU,baseKva)) + " " );
			if (x.getFlowControlType() == FlowControlType.POINT_CONTROL)
				str.append( "   " + Number2String.toStr("##0.0000", x.getMvarSpecified(UnitType.PU,baseKva)) + "    " );
			else
				str.append( x.getControlRange() + " " );
		}

		str.append( Number2String.toStr("0.000", (x.isControlOnFromSide()? x.getAclfBranch().getFromTurnRatio() :
														x.getAclfBranch().getToTurnRatio())) + " " );
...
public static String psXfrPControlToString(AclfAdjNetwork net) throws Exception {
	StringBuffer str = new StringBuffer("");
	...
	str.append( "          BranchID       Pact     Spec/Range    Ang   Max   Min  Status\n" );
	str.append( "     ----------------- -------- -------------- ----- ----- ----- ------\n" );

	for( PSXfrPControl x : net.getPsXfrPControlList()) {
		str.append( Number2String.toStr(5, " ") );
		str.append( Number2String.toStr(-17, x.getAclfBranch().getId() ) + " ");
		str.append( Number2String.toStr("##0.0000", (x.isControlOnFromSide()?
							x.getAclfBranch().powerFrom2To(UnitType.PU,baseKVA).getReal() :
							x.getAclfBranch().powerTo2From(UnitType.PU,baseKVA).getReal())) + " " );

		if (x.getFlowControlType() == FlowControlType.POINT_CONTROL)
			str.append( Number2String.toStr("   " + "##0.0000", x.getPSpecified(UnitType.PU,baseKVA)) + "    " );
		else
			str.append( x.getControlRange() + " " );

		PSXfrAdapter psXfr = (PSXfrAdapter)x.getAclfBranch().adapt(PSXfrAdapter.class);
		str.append( Number2String.toStr("#0.00", psXfr.getFromAngle(UnitType.Deg)) + " " );
		str.append( Number2String.toStr("#0.00", x.getAngLimit(UnitType.Deg).getMax()) + " " );
		str.append( Number2String.toStr("#0.00", x.getAngLimit(UnitType.Deg).getMin()) + " " );
		str.append( Number2String.toStr(6, x.isActive() ? "on" : "off" ) + "\n");
	}
	return str.toString();
}
