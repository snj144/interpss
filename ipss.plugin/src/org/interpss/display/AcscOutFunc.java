...
public class AcscOutFunc {
	public static String faultResult2String(SimpleFaultNetwork faultNet) {
		...
		IpssLogger.getLogger().fine("# of Fault objects = "+faultNet.getFaultList().size());
	...
	private static String branchFaultInfo( AcscBranchFault bf, AcscNetwork net )	{
		...
		try {
			...
			str.append( "          Branch name:   " + bf.getFaultBranch().getName() + "\n");
			str.append( "          Fault type:    " + bf.getFaultCode() + "\n");
			str.append( "          Fault current: " + ComplexFormat.formatComplex(bf.getFaultResult().getSCCurrent_012().b_1) + " pu\n" );
			str.append( "                         " + 
					ComplexFormat.formatComplex(bf.getFaultResult().getSCCurrent_012(UnitType.Amp, baseV, baseKVA).b_1) + " amps\n" );
			if ( bf.getFaultResult().getZFault(UnitType.Ohm, baseV, baseKVA).abs() > 0.0 )
				str.append( "          Fault z:       " +
						ComplexFormat.formatComplex(bf.getFaultResult().getZFault(UnitType.Ohm, baseV, baseKVA)) + " ohms\n" );
	...
	private static String displayBusVoltage(AcscBusFault bf, AcscNetwork net) {
		...
		if (bf.getFaultCode() == SimpleFaultCode.GROUND_3P) {
			...
			for( Bus b : net.getBusList()) {
				...
				double vpu = bf.getFaultResult().getFaultVoltage_012(bus.getSortNumber()).b_1.abs();
				Complex3x1 ampPu = bf.getFaultResult().getContriAmps_012(bus.getSortNumber());
				Complex3x1 amps  = bf.getFaultResult().getContriAmps_012(bus.getSortNumber(), UnitType.Amp, 
										bus.getBaseVoltage(), net.getBaseKva());
				str.append( Number2String.toStr("#####0.###", vpu ) + "   ");
				str.append( Number2String.toStr("#######0.#", vpu*bus.getBaseVoltage()));
			...
			for( Bus b : net.getBusList()) {
				...
				str.append( Number2String.toStr("#######0.#", vpu1*bus.getBaseVoltage() ) + "");
				double vpu0 = v012.a_0.abs();
				str.append( Number2String.toStr("#####0.###", vpu0 ) + "   ");
				str.append( Number2String.toStr("#######0.#", vpu0*bus.getBaseVoltage() ) + "");
				double vpu2 = v012.c_2.abs();
				str.append( Number2String.toStr("#####0.###", vpu2 ) + " ");
				str.append( Number2String.toStr("#######0.#", vpu2*bus.getBaseVoltage() ) + "\n");
			}
		...
		if (bf.getFaultCode() == SimpleFaultCode.GROUND_3P) {
			...
			for ( int n = 0; n < branchList.size(); n++ ) {
				...
				try {
					...
					str.append(Number2String.toStr("###0.###", cpu.b_1.abs()) + "   " + Number2String.toStr("#######0.#", camp.b_1.abs()) + "\n");
				} catch (Exception e) {
					IpssLogger.logErr(e);
			...
			for ( int n = 0; n < branchList.size(); n++ ) {
				...
				try {
					...
					str.append( "     " + Number2String.toStr(-20, bra.getId() ) + "   ");
					str.append(Number2String.toStr("###0.###", cpu.b_1.abs()) + "   " + Number2String.toStr("#######0.#", camp.b_1.abs()) + "   ");
					str.append(Number2String.toStr("###0.###", cpu.a_0.abs()) + "   " + Number2String.toStr("#######0.#", camp.a_0.abs()) + "   ");
					str.append(Number2String.toStr("###0.###", cpu.c_2.abs()) + "   " + Number2String.toStr("#######0.#", camp.c_2.abs()) + "\n");
					...
					str.append("     " + Number2String.toStr(-9, " ") + "<-" + Number2String.toStr(-9, " ") + "   ");
					str.append(Number2String.toStr("###0.###", cpu.b_1.abs()) + "   " + Number2String.toStr("#######0.#", camp.b_1.abs()) + "   ");
					str.append(Number2String.toStr("###0.###", cpu.a_0.abs()) + "   " + Number2String.toStr("#######0.#", camp.a_0.abs()) + "   ");
					str.append(Number2String.toStr("###0.###", cpu.c_2.abs()) + "   " + Number2String.toStr("#######0.#", camp.c_2.abs()) + "\n");
				} catch (Exception e) {
