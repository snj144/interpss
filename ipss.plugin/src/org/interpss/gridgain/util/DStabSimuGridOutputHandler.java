...
/**
 * DStab output message handler. The simulation result messages are re-sent to
 * the master node using the msgHub object (IPSSGridMsgHubImpl)
 */

public class DStabSimuGridOutputHandler extends AbstractSimuOutputHandler {
	/**
	 * IPSSGridMsgHubImpl, a message router which routes message to the master
	 * node
	 */
	 ...
	 * 
	 * @param msgHub
	 *            a message router object, which routes the message to the
	 *            master node
	 * @param caseId
	 *            remote grid node run case.id = net.id
	 */
